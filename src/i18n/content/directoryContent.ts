...
export const directoryContent: Record<SupportedLanguage, DirectoryContent> = {
  ...
  ja: {
    ...
    aboutP2: 'SciCalcXのすべての数学計算ツール（関数電卓、行列代数、微積分エンジン、2D関数プロッター、記述統計アナライザー、32ビットプログラマービットボード）は100%クライアントサイドで動作します。IEEE-754倍精度浮動小数点演算、合成シンプソン則による数値積分、ラプラス展開、ネイティブビット演算を用い、数式やデータを外部サーバーに一切送信することなく、お使いの端末内で瞬時に計算を実行します。',
    ...
    stages: [
      ...
      },
      {
        ...
        desc: '生データ（float64）に対し、IEEE-754特有の微小誤差（0.1 + 0.2 = 0.30000000000000004など）を除去し、数学的に厳密で視認性の高い結果を表示します。',
      },
    ],
    privacyTitle: 'プライバシー保護と厳格な編集方針',
    privacyText: 'SciCalcXは、ユーザーが入力した数式、行列データ、統計サンプルを記録・追跡・収益化することはありません。計算履歴はお使いのブラウザのlocalStorageにのみ安全に保存されます。すべての学習記事や技術ドキュメントは大学の標準的教科書に基づいて検証されています。',
  ...
  nl: {
    ...
    aboutP2: 'Alle wiskundige onderdelen op SciCalcX (Wetenschappelijke Rekenmachine, Matrixalgebra, Calculus, 2D Grafieken, Beschrijvende Statistiek en Programmeurs-Bitboard) functioneren 100% client-side. Dankzij IEEE-754 dubbele precisie, numerieke integratie (Simpson-regel), Laplace-expansie en native bitbewerkingen worden berekeningen direct op uw apparaat uitgevoerd zonder dat er gegevens naar externe servers gaan.',
  ...
  pt: {
    ...
    aboutP2: 'Todas as ferramentas matemáticas do SciCalcX (Calculadora Científica, Álgebra Matricial, Cálculo, Gráficos 2D, Estatística Descritiva e Bitboard de Programador) operam 100% no cliente. Com aritmética de ponto flutuante IEEE-754 de precisão dupla, integração numérica (regra de Simpson), expansão de Laplace e operações bit a bit nativas, seus cálculos ocorrem diretamente no seu navegador sem envio de dados a servidores externos.',
    ...
    stages: [
      ...
      },
      {
        ...
        desc: 'Os resultados brutos em float64 passam por normalização de precisão para evitar artefatos binários da norma IEEE-754 (como 0.1 + 0.2), garantindo resultados matematicamente limpos.',
      },
    ],
    privacyTitle: 'Nosso Compromisso com a Privacidade e o Rigor Editorial',
    privacyText: 'O SciCalcX não rastreia, monetiza nem armazena as suas equações matemáticas, matrizes ou amostras estatísticas. O histórico de cálculo é gravado exclusivamente no armazenamento local (localStorage) do seu próprio navegador. Todo o conteúdo didático e as fórmulas são validados com base na bibliografia acadêmica padrão.',
  ...
  ko: {
    ...
    aboutP2: 'SciCalcX의 모든 수학 도구(공학용 계산기, 행렬 대수 스위트, 미적분 엔진, 2D 함수 그래프, 기술 통계 분석기, 32비트 프로그래머 비트보드)는 100% 클라이언트 사이드에서 작동합니다. IEEE-754 배정밀도 부동소수점 산술, 심슨 공식을 이용한 수치 적분, 라플라스 여인수 전개 및 네이티브 비트 연산을 활용하여, 입력한 수식이나 데이터셋을 외부 서버로 전송하지 않고 브라우저 내부에서 즉시 처리합니다.',
    ...
    stages: [
      ...
      },
      {
        ...
        desc: '원시 float64 연산 결과에서 발생할 수 있는 IEEE-754 2진 부동소수점 오차(예: 0.1 + 0.2 = 0.30000000000000004)를 정규화하여 수학적으로 정확하고 깔끔한 결과를 표시합니다.',
      },
    ],
    privacyTitle: '철저한 개인정보 보호 및 편집 원칙',
    privacyText: 'SciCalcX는 사용자가 입력한 수학 방정식, 행렬 데이터, 통계 표본을 기록하거나 추적하지 않습니다. 계산 기록은 사용자의 브라우저 localStorage에만 로컬로 안전하게 저장됩니다. 모든 교육 문서와 공식은 대학 표준 교과서에 근거하여 엄격히 검증되었습니다.',
  ...
  it: {
    ...
    aboutP2: 'Tutti gli strumenti matematici di SciCalcX (Calcolatrice Scientifica, Algebra Matriciale, Analisi Matematica, Grafici 2D, Statistica Descrittiva e Bitboard per Programmatori) funzionano al 100% lato client. Con l\'aritmetica in virgola mobile IEEE-754 a doppia precisione, l\'integrazione numerica con regola di Simpson, lo sviluppo di Laplace e operazioni bit a bit native, i calcoli avvengono direttamente sul dispositivo senza inviare dati a server remoti.',
    ...
    stages: [
      ...
      },
      {
        ...
        desc: 'I risultati grezzi float64 vengono normalizzati per prevenire artefatti di rappresentazione binaria IEEE-754 (come 0.1 + 0.2), garantendo una visualizzazione rigorosa e pulita.',
      },
    ],
    privacyTitle: 'Il Nostro Impegno per la Privacy e il Rigore Editoriale',
    privacyText: 'SciCalcX non registra, non monetizza e non invia a terzi le vostre equazioni matematiche, matrici o serie di dati. La cronologia dei calcoli risiede unicamente nella memoria locale (localStorage) del vostro browser. Tutti i contenuti e gli esempi didattici sono verificati rispetto ai testi accademici di riferimento.',
